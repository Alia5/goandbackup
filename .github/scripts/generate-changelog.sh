...
# Usage: generate-changelog.sh <output_file> [tag]
...
for commit_hash in "${COMMITS[@]}"; do
  ...
  if [[ -z "$changelog_type" ]] && echo "$commit_body" | grep -iqE 'changelog[(: ]'; then
...

{
  ...
  if [[ -z "$FEATURES" && -z "$FIXES" && -z "$MISC" ]]; then
    echo "No categorized changelog entries found."
  fi
} > "$OUTPUT_FILE"
